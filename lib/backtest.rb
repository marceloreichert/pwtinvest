...
#    cot = cotacoes.at(indice + (position_candle - 1))
  ...
  def self.verify( ticks,
    ...
    ret = Backtest.find(ticks, indice, setup[:id], quantidade_maxima_de_candles_do_trade)

    return nil if not ret[:find]
    return nil if not Backtest.validate_relation(ret[:candles_on_setup], setup[:id])
