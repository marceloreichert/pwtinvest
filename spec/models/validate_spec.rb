...
describe Validate do
  ...
  it 'load_setup_types' do
    types = Validate.load_setup_types(1)
  ...
  it 'filter' do
    ...
    filtered  = Validate.filter(ticks, ['B', 'A'])
    expect(filtered.size).to eq 2
    expect(filtered[0]).to eq [0, 1]
    expect(filtered[1]).to eq [3, 4]
  end

  it 'Validate.validate(@ticks, params[:setup][:id])' do
    ticks = DailyQuotation.where('paper = ?', 'PETR4.SA')
    t = Validate.validate(ticks, 1)
